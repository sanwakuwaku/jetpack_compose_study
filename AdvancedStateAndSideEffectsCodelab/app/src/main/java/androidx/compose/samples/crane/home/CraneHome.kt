...
fun CraneHome(
    ...
    Scaffold(
        ...
        // Compositionの外でCoroutineScopeを使えるように保持する
        val scope = rememberCoroutineScope()
        CraneHomeContent(
            modifier = modifier.padding(padding),
            onExploreItemClicked = onExploreItemClicked,
            openDrawer = {
                // openがsuspend functionだがコールバック内はCompositionから
                // 出ているのでLaunchEffectは使用できない。そのためCoroutine scopeを使う必要がる
                scope.launch {
...
fun CraneHomeContent(
    ...
    // ComposeのState APIに変換されて、Composeからトラッキングされるようになる。
