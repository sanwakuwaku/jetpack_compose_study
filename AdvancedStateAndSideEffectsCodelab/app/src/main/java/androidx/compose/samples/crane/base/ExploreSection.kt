    ...
    Surface(modifier = modifier.fillMaxSize(), color = Color.White, shape = BottomSheetShape) {
        Column(modifier = Modifier.padding(start = 24.dp, top = 20.dp, end = 24.dp)) {
            ...
            Box(Modifier.weight(1f)) {
                ...
                // 特定の状態が他の状態から計算される場合はderivedStateOfを使用する。
                // 計算で使用される状態のいずれかが変化した時のみ計算が行われることが保障される。
                // さらに、状態が宣言されているコンポーザブルの再コンポジションは発生せず、
                // 状態が読み取られるコンポーザブルのみを再コンポーズする
                val showButton by remember {
                    derivedStateOf {
                        // このブロックはfirstVisibleItemIndex(State)が変更される度に呼び出されるが、
                        // 再コンポーズされるのは式の値が変更になった場合のみ
                        listState.firstVisibleItemIndex > 0
                    }
                }

                // derivedStateOfを使わないとfirstVisibleItemIndexが変化する度に再コンポーズが走ってしまう
